...
2. 新电脑装好 TagTerm，到「设置 → 导入导出」点「导入配置…」选这份文件：标签、唤起命令、全局快捷键、开机自启、Agent hooks 开关、外观参数和终端字号就位。标签按名合并（同名沿用本机的，不删本机独有的），其余整体替换；导入前会先把当前配置自动备份到 `%APPDATA%\TagTerm\backups\`，导错了把备份再导入一次即可。
