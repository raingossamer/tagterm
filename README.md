...
- **会话即路径**：新建会话只需选一个目录（名称缺省取目录末段，可选 cmd.exe / PowerShell / pwsh），列表持久化在 `%APPDATA%\TagTerm\sessions.json`。右键会话可编辑名称 / 目录 / Shell（终端里有程序在跑时拒绝改目录 / Shell，空闲则自动在新目录重启）；行右侧的垃圾桶移除会话（二次确认）。
...
- **设置**：终端背景图（自选图片 + 黑色遮罩滑块，保证文字可读）；开机自启（登录后静默进托盘，不弹窗口）；关于（版本、数据目录）。
