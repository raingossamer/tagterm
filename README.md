...
- **导入导出配置**：「设置 → 导入导出」把标签、唤起命令、全局快捷键、开机自启、Agent hooks 开关、外观参数和终端字号导出成一份 JSON，换电脑后导入即可；标签按名合并，其余整体替换，导入前自动备份当前配置，每一项的结果逐条列出。不含会话、会话的标签和背景图片（见下文「换电脑迁移」）。
...
**从 0.3.10 或 0.3.11 升级**：这两个版本的「检查更新」有缺陷，查不到新版本。请到 Releases 手动下载 0.3.12 或更新版本的安装包装一次，之后应用内更新恢复正常。

装好后建议到「设置 → Agent」打开 Claude Code / Codex 的 hooks 开关，状态点会准确很多。以后升级到新版本时，已打开的 hooks 会在启动时自动更新到新版本，不用再手动开关。

### 换电脑迁移

1. 旧电脑上到「设置 → 导入导出」点「导出配置…」，得到一份 `tagterm-config-<日期>.json`。
2. 新电脑装好 TagTerm，到「设置 → 导入导出」点「导入配置…」选这份文件：标签、唤起命令、全局快捷键、开机自启、Agent hooks 开关、外观参数和终端字号就位。标签按名合并（同名的沿用本机的标签与关联，颜色和是否显示以文件为准；本机独有的不删），其余整体替换；导入前会先把当前配置自动备份到 `%APPDATA%\TagTerm\backups\`，导错了把备份再导入一次即可。
3. 会话不随文件走（目录路径跟着机器与用户名变），在新电脑上按新路径重新建；背景图片也不带，到「设置 → 外观」重新选一张。

hooks 在新电脑上装不上时（比如还没装 Claude Code），结果里会写明原因，装好后到「设置 → Agent」再打开。只导入自己导出或信任的文件：唤起命令会原样写进终端。要整份搬家（连会话一起），仍可退出 TagTerm 后把 `%APPDATA%\TagTerm\` 下的 `sessions.json`、`settings.json`、`tags.json` 拷到新电脑的同一位置。

## 数据与隐私

- 应用自己的数据只有三个 JSON 文件，都在 `%APPDATA%\TagTerm\`：`sessions.json`（会话）、`settings.json`（唤起命令、背景与全局快捷键）、`tags.json`（标签与关联）。卸载不会删除它们。导入配置前的自动备份在同目录的 `backups\` 下，只留最近 10 份。
...
- 关掉的标签页只保留最近 200 行输出（省内存），重开后更早的历史翻不到；开着的标签页仍保留 5000 行。
- 安装包未签名。
