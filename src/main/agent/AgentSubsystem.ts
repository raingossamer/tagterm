...
export interface AgentSubsystemOptions {
  ...
  /** 缺省 derivePort(dataDir)，整段起不来再依次换 hookPortFallbacks；测试注入的端口只试它（0 = 随机端口） */
  preferredPort?: number
  /** 每段端口最多试几个（测试用 1 构造「全部失败」） */
...
export class AgentSubsystem {
  ...
  async start(): Promise<void> {
    try {
      // 注入了端口（测试）只试它；否则数据目录派生的首选段起不来时依次换兜底起点
      const { preferredPort, dataDir } = this.opts
      this.hookPort =
        preferredPort !== undefined
          ? await this.hookServer.start(preferredPort)
          : await this.hookServer.start(derivePort(dataDir), hookPortFallbacks(dataDir))
 ...
 * 输入按小写、统一正斜杠归一，同一目录的两种写法得到同一端口。被占用时 HookServer 自己 +1 顺延。
 * seed 非 0 时派生另一个端口（兜底起点，见 hookPortFallbacks）；0 与加种子之前的派生结果一致。导出只为直接断言派生规则
 */
export function derivePort(dataDir: string, seed = 0): number {
  const normalized = dataDir.replace(/\\/g, '/').replace(/\/+$/, '').toLowerCase()
  const key = seed === 0 ? normalized : `${normalized}#${seed}`
  let hash = 0x811c9dc5
...
/**
 * 首选端口段（derivePort 起顺延 maxPortAttempts 个）整段起不来时依次换的起点 —— Windows 的动态保留端口段
 * （Hyper-V / WSL 按 100 个一段划走，重启可能变）会把整段盖住，全部 EACCES、hooks 就静默不可用。
 * 先换两个种子再派生：同一数据目录每次启动算出同一串，保留段没变时端口就不变，已装的 hooks 不必每次启动改写一遍、
 * 多一份备份；最后才是随机端口（系统分配，不会落进保留段）
 */
export function hookPortFallbacks(dataDir: string): number[] {
  return [derivePort(dataDir, 1), derivePort(dataDir, 2), 0]
}
