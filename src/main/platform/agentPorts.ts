 ...
 * tray / window / overlay 图都以 getter 传入：它们在 whenReady 里才创建（子系统在 ready 之前就装配好了）、before-quit 会置空
 */
...
export interface ElectronBadgeDeps {
  ...
  /** 任务栏 overlay 的黄点 / 蓝点 / 绿点图（resources/overlay-blocked.png / overlay-done.png / overlay-working.png，16×16）；ready 之前为 null */
  overlays: () => { blocked: NativeImage; done: NativeImage; working: NativeImage } | null
}
...
export function electronBadge(deps: ElectronBadgeDeps): BadgePort {
  return {
    setCounts(counts) {
      ...
      if (!win || win.isDestroyed() || !overlays) return
      if (counts.blocked > 0) {
        win.setOverlayIcon(overlays.blocked, `${counts.blocked} 个会话等你确认`)
      } else if (counts.done > 0) {
        win.setOverlayIcon(overlays.done, `${counts.done} 个会话已完成`)
      } else if (counts.working > 0) {
        win.setOverlayIcon(overlays.working, `${counts.working} 个会话运行中`)
      } else {
