 ...
 *   electronBadge：托盘图标（TrayHandle.setBadge，计数没变它自己不重复设置）+ 任务栏按钮 overlay（setOverlayIcon），
 *     两处都按 等你确认（黄点）> 运行中（绿点）> 无 的优先级选图
...
export interface ElectronBadgeDeps {
  ...
  /** 任务栏 overlay 的黄点 / 绿点图（resources/overlay-blocked.png / overlay-working.png，16×16） */
  overlays: { blocked: NativeImage; working: NativeImage }
}
